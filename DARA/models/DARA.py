...
from models.adapter import RA_Adapter

class DARA(nn.Module):
    def __init__(self, args,config):
        super(DARA, self).__init__()
        hidden_dim = args.vl_hidden_dim
        divisor = 16 if args.dilation else 32
        self.num_visu_token = int((args.imsize / divisor) ** 2)
        self.num_text_token = args.max_query_len

        # share adapters for relation-aware
        shared_adapters = nn.ModuleList([
            RA_Adapter(bottleneck=args.RA_bottleneck,up_dim=256)
            for i in range(6)])
        

        self.visumodel = build_detr(args,vis_adapters = shared_adapters)
        self.textmodel = build_bert(args,config,text_adapters = shared_adapters)
